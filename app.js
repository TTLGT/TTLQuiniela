...
const TEAM_FLAGS = {
  ...
  'Gales': 'gb-wls', 'Irlanda': 'ie', 'Irlanda del Norte': 'gb-nir',
  'Albania': 'al', 'Bulgaria': 'bg', 'Eslovaquia': 'sk', 'Eslovenia': 'si',
  'Rusia': 'ru', 'Bielorusia': 'by',
  'Japon': 'jp', 'Corea del Sur': 'kr', 'China': 'cn', 'India': 'in',
  'Australia': 'au', 'Nueva Zelanda': 'nz', 'Qatar': 'qa', 'Arabia Saudita': 'sa',
  'Iran': 'ir', 'Irak': 'iq', 'Jordania': 'jo',
  'Uzbekistan': 'uz', 'Emiratos Arabes': 'ae',
  'Tailandia': 'th', 'Vietnam': 'vn', 'Singapur': 'sg',
...
async function renderPhase(phaseId) {
  ...
  const participantHeaders = participants.map((p, i) => {
    ...
    const flagSpan = flagHtml ? `<span style="display:block;line-height:1.4;font-size:0.9rem">${flagHtml}</span>` : '';
    const nameHtml  = lastName
      ? `${flagSpan}<span style="display:block;line-height:1.2;font-size:0.62rem">${firstName}</span><span style="display:block;line-height:1.2;font-size:0.55rem">${lastName}</span>`
      : `${flagSpan}<span style="display:block;line-height:1.2;font-size:0.62rem">${firstName}</span>`;
